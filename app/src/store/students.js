...
const students = {
  ...
  actions: {
    ...
    fetchCurrentStudents({ commit }, args) {
      return Vue.http
        .get(`students/current/?desc=true`, {
          Authorization: Vue.http.headers.common.Authorization,
        })
        .then(response => {
          commit('setStudents', response.body);
    ...
    searchStudents({ commit }, args) {
      ...
      return Vue.http
        .get(url, { Authorization: Vue.http.headers.common.Authorization })
        .then(response => {
    ...
    updateStudentDetail({ commit }, studentId, studentDetails) {
      console.log('new deets', studentDetails);
      return Vue.http
        .patch(`students/update/${studentId}`, studentDetails, {
          Authorization: Vue.http.headers.common.Authorization,
        })
        .then(response => {
          commit('setStudentDetail', response.body);
